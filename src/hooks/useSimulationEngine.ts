...
import { Candle, SimulationState, SymbolType, TimeframeType, TraderProfile } from '../types';
import { TF_SECONDS } from '../constants';
import { fetchHistoricalData, fetchContextCandles, fetchFutureCandles, fetchFirstCandle } from '../services/api';
...
export function useSimulationEngine(
  ...
  useEffect(() => {
     ...
     const loadInitialData = async () => {
         ...
         setIsLoading(true); setShowDataError(false); setHasReachedEnd(false);
         try {
  ...
  // Stream Buffering
  useEffect(() => {
      if (!chartData.length || isLoadingFuture || hasReachedEnd) return;
          ...
          const loadFuture = async () => {
              ...
              if (moreData.length > 0) {
                  setChartData(prev => {
                       ...
                       if (newUnique.length === 0) { setHasReachedEnd(true); return prev; }
  ...
  }, [simState.currentIndex, chartData, activeSymbol, activeTimeframe, isLoadingFuture, hasReachedEnd]);
